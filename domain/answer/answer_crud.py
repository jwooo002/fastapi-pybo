...
def select_answer(db: Session, answer_id: int):
    return db.query(Answer).get(answer_id) #답변 객체 리턴?
...
def recommend_answer(db: Session, db_answer: Answer, db_user: User):
    db_answer.recommend.append(db_user)
    db.commit()